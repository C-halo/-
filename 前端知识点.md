...
//基本数据类型number,string,null等会返回新建的对象；若构造函数返回引用类型的，则fn指向返回的引用类型
```
...
```
这里少了Foo.prototype 和Function.prototype指向Object.prototype的线
```
...
```
hash就是#+后面的内容，hash虽然出现 URL 中，但不会被包含在 HTTP 请求中，对后端完全没有影响，因此改变 hash 不会重新加载页面。
...
![img](C:\Users\halo\Desktop\前端知识点.assets\640)
...
```
面向连接可靠的，能够保证数据无差错，不丢失，不重传，按序的到达，只能一对一，面向字节流，对数据可靠性要求很高使用，比如浏览器和服务器的连接。
```

UDP用户数据协议

```
无连接不可靠的，只能尽最大可能交付数据，可以一对一或者一对多，面向报文，使用与对高速传输数据和实时性，比如视频会议
```
...
```
1.客户端发起请求报文：SYN=1,seq=x,此时不能携带数据
2.服务器接收到发起确认报文：SYN=1,seq=y,ack=x+1,ACK=1，此时不能携带数据
3.客户端收到后也许要发起确认报文：ack=y+1，seq=y+1,ACK=1，此时可以携带数据
...
```
1.客户段发送请求结束报文，FIN=1,seq=u（最后一个字节+1）
2.服务器发送确认报文，ACK=1,ack=u+1,seq = v  ,但是只是不再接收客户端数据，自己还要发送未发完的数据
3.等到服务器的数据发完了，发送请求结束报文，FIN=1,seq=w(这时候已经与v不同了)，ack=u+1,ACK=1
4.客户端发送确认关闭报文，ACK=1,ack=w+1,seq = u+1,然后再等2*最长报文段寿命再关闭

为什么要等2*最长报文段？
	1.保证最后的确认能够达到服务器，若因为丢失，服务器一段时间没有收到会重复第3步，这样客户端能够重启2*MSL
	2.避免之前发送的报文重新传给了服务器，经过2*MSL,所有报文段都消失了
为什么要四次挥手？
	服务区收到客户端发送的关闭请求，只是代表自己可以不再接收数据了，但是可能服务器还有未发送完的数据，所以先发送确认，等自己的数据发完了，再发一个关闭报文，导致比三次多了一次。
```
...
3.浏览器得到后，开始构建dom树，遇到图片音视频并行下载，遇到js会只下载js阻塞dom树的构建;构建cssom树，然后和dom树合并成渲染树。
4.确定页面的布局，渲染页面，展示给用户
5.涉及到缓存机制。
```
...
绝对定位和margin-left: -width/2, 前提是父元素position: relative，left:50%;
...
let quick = (arr, start, end) => {
    ...
    for (let i = start + 1; i <= end; i++) {
        if (arr[i] < pivot) {
            mark++;//既pivot最终的位置
            [arr[i], arr[mark]] = [arr[mark], arr[i]];//互换位置
        }
    }
    arr[start] = arr[mark];//最后与其互换
    arr[mark] = pivot; //让mark等于pivot所在位置
    return mark;
}
let findK = (arr, k) => {
...
let findK = (arr, k) => {
    ...
    let quick = (arr, begin, end) => {
        ...
        else if (j < n - k) return quick(arr, j + 1, end);
        else return quick(arr, begin, j - 1);
    }
...
var getLeastNumbers = function (arr, k) {
    let quicksort = (arr, begin, end) => {
        ...
        arr[begin] = arr[j];
        arr[j] = base;
        if (j === k - 1) return arr.slice(0, k);//假如刚好该下标是k-1，返回前k个即可
        else if (j > k - 1) {
            return quicksort(arr, begin, j - 1);//需要对左边的进行排序
        } else {
            return quicksort(arr, j + 1, end);//需要对右边的进行排序
        }
    }
...
class LazyMan {
    constructor(name) {
        ...
        setTimeout(() => {
            this.next();//必须setTimeOut,使得next是宏任务
        }, 0);
    }
    eat(food) {
